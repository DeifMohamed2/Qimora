...
function brandHeaderHtml(logoImgSrc, siteUrl) {
  return `<tr>
  <td class="qim-header" style="padding:0;margin:0;background:${P.headerGrad3};background:linear-gradient(155deg,${P.headerGrad1} 0%,${P.headerGrad2} 42%,${P.headerGrad3} 100%);border-radius:18px 18px 0 0;border:1px solid ${P.headerBorder};border-bottom:none;">
    <table role="presentation" width="100%" cellspacing="0" cellpadding="0" bgcolor="${P.headerGrad3}">
      <tr>
        <td align="center" style="padding:34px 24px 26px;">
          <a href="${attrSafe(siteUrl)}/" target="_blank" rel="noopener noreferrer" style="text-decoration:none;display:inline-block;color:${P.accentSoft};" title="Qimora">
            <span style="display:inline-block;padding:12px 16px;background:rgba(6,10,18,0.55);border-radius:14px;border:1px solid rgba(130,170,255,0.28);box-shadow:0 4px 24px rgba(0,0,0,0.35);">
              <img src="${attrSafe(logoImgSrc)}" width="168" height="34" alt="Qimora — build once, scale fast" style="display:block;margin:0 auto;border:0;outline:none;height:34px;width:168px;max-width:100%;-ms-interpolation-mode:bicubic;" />
            </span>
          </a>
          <p style="margin:16px 0 0;font-size:10px;font-weight:700;letter-spacing:0.22em;text-transform:uppercase;color:${P.accentSoft};">Build once · Scale fast</p>
        </td>
      </tr>
    </table>
  </td>
