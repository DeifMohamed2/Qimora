...
function meetingConfirmedGuestHtml(doc, labels, meetLink, siteUrl, logoImgSrc) {
  ...
  <tr><td style="font-size:13px;word-break:break-all;">
    <a href="${safeHref}" style="color:#2563eb;text-decoration:underline;">${esc(meetLink)}</a>
  </td></tr>
</table>`
    : `<p style="margin:0 0 22px;padding:14px 16px;background:#f1f5f9;border-radius:10px;border:1px solid #e2e8f0;color:#475569;font-size:14px;line-height:1.55;">Your meeting is confirmed. If a Meet link was not generated automatically, we will follow up with joining details.</p>`;

  const inner = `
<h1 style="margin:0 0 12px;font-size:21px;line-height:1.3;color:#0f172a;font-weight:700;">Your meeting is confirmed</h1>
...
${meetBlock}
<p class="eb-accent-label" style="margin:0 0 10px;font-size:12px;font-weight:700;letter-spacing:0.1em;text-transform:uppercase;color:#2563eb;">Agenda</p>
<p style="margin:0 0 20px;padding:16px 18px;background:#f8fafc;border-radius:12px;border:1px solid #e2e8f0;color:#334155;font-size:15px;line-height:1.6;white-space:pre-wrap;">${esc(doc.agenda)}</p>
${doc.company ? `<p style="margin:0 0 18px;color:#475569;font-size:14px;"><strong style="color:#0f172a;">Company:</strong> ${esc(doc.company)}</p>` : ''}
<p style="margin:0;color:#64748b;font-size:13px;line-height:1.55;">Please join a few minutes early to check audio and video. To reschedule, reply to this email.</p>
<p style="margin:22px 0 0;font-size:14px;color:#475569;">Best regards,<br /><strong style="color:#0f172a;">The Qimora team</strong></p>
`;
